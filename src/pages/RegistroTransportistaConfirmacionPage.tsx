...
export type RegistroTransportistaConfirmState = {
  nombre?: string
  email?: string
...
export function RegistroTransportistaConfirmacionPage() {
  const state = useLocation().state as RegistroTransportistaConfirmState | null
  const nombre = state?.nombre?.trim() || null
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50 text-slate-900">
      ...
      <main className="mx-auto flex w-full max-w-lg flex-1 flex-col items-center px-4 py-12 text-center sm:px-6 sm:py-16">
        ...
        <p className="mb-2 text-xs font-bold uppercase tracking-widest text-emerald-700">Solicitud enviada</p>
        <h1 className="mb-4 text-xl font-extrabold text-slate-800 sm:text-2xl">¡Listo, gracias!</h1>
        <p className="mb-4 rounded-xl border border-emerald-100 bg-emerald-50/80 px-4 py-3 text-left text-sm leading-relaxed text-slate-700">
        ...
        <p className="mb-2 text-slate-600 leading-relaxed">
          Registramos tu solicitud para <strong>ofrecer servicios</strong>
          {nombre ? (
            <>
              {' '}
        ...
        {!email && <p className="mb-8 text-sm text-slate-500">Te contactaremos a la brevedad.</p>}

        <div className="flex w-full max-w-sm justify-center">
          <Link
            to="/"
            className="rounded-xl bg-slate-800 px-8 py-3 text-center text-sm font-semibold text-white transition hover:bg-slate-900"
          >
            Volver al inicio
          </Link>
        </div>
